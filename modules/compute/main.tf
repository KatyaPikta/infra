...
resource "digitalocean_droplet" "control_plane" {
  ...
  name     = "master" # static name for "count" resource
  image    = var.os_image
  region   = var.region
  ...
  user_data = templatefile("${path.module}/templates/cloud-init.yaml", {
    node_type   = "control-plane" # what is the use for these parameters for this templatefile?
    environment = var.environment
    hostname    = "${var.project_name}-${var.environment}-cp-${count.index + 1}"
  })
}

resource "digitalocean_droplet" "workers" { # better to use singular naming (worker)
